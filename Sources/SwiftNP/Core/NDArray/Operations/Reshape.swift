...
public extension NDArray {
    ...
    ///   - order: The order in which the array should be reshaped, either C-style or Fortran-style (default is C-style).
    ...
    func reshape(to shape: Shape, order: ReshapeOrder = .c) throws(SNPError) -> NDArray {
        ...
        if shape.count == 1 {
            return NDArray(shape: shape, dtype: self.dtype, data: flattenedArray)
        }
